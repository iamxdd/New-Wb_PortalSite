$(document).ready(function() {
	...
	industrylist();
var reciveId=GetQueryString("reciveId");
	/*获取我的个人信息*/
	var getMyInfo = function() {
		var _url=window.url.gertalentbyid+"?id="+reciveId;
		GetAjax(_url, _header, ".sp-lefts").done(function(res) {
			if (res.State.Code == 0) {
				...
				if (baseInfo.ExpectedJob && baseInfo.ExpectedJob.length > 0) {
					...
					ExpectedJobHtml =
						...
						'<p class="clearfix"><b>工作地点：' + baseInfo.ExpectedJob[0].ExpectedWorkingPlaceName + ' </b><b>期望企业：' + numberToText(baseInfo.ExpectedJob[0].EnterpriseNature, EnterpriseNatures) + '</b></p>';
